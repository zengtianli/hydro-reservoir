...
# 页面配置
# ============================================================
page_config("水库群调度 - Hydro Toolkit", "⚡")
...
if 'input_data' not in st.session_state:
    st.session_state.res_input_data = None  # 存储解析后的输入数据
if 'calculated' not in st.session_state:
    st.session_state.res_calculated = False
if 'result_xlsx' not in st.session_state:
    st.session_state.res_result_xlsx = None  # 存储结果文件的 bytes
if 'up_res' not in st.session_state:
    st.session_state.res_up_res = None
if 'down_res' not in st.session_state:
    st.session_state.res_down_res = None
...
def parse_uploaded_xlsx(uploaded_file) -> dict:
    ...
    if '计算参数' in xlsx.sheet_names:
        ...
        for _, row in df_params.iterrows():
            ...
            if param_name == '上库':
                data['res_up_res'] = str(row.iloc[1]).strip() if pd.notna(row.iloc[1]) else None
            elif param_name == '下库':
                data['res_down_res'] = str(row.iloc[1]).strip() if pd.notna(row.iloc[1]) else None
            elif param_name:
...
def run_calculation_from_xlsx(uploaded_file, calc_step: str = "旬") -> bytes:
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        up_res = result['res_up_res']
        down_res = result['res_down_res']
        ...
        # 读取输出列名
        output_list_file = Path(__file__).resolve().parent / "src" / "reservoir" / "output_columns.csv"
        ...
        up_table, down_table = test.power_operate_year_up_down(
            ...
            up_res_name=paras_dict['res_up_res'],
            down_res_name=paras_dict['res_down_res'],
...
with st.sidebar:
    ...
    if uploaded_file:
        try:
            st.session_state.res_input_data = parse_uploaded_xlsx(uploaded_file)
            st.success("✅ 文件解析成功")
            
            data = st.session_state.res_input_data
            st.markdown(f"""
            **识别到的水库：**
            - 🏔️ 上游：{data['res_up_res']}
            - 🌊 下游：{data['res_down_res']}
            
            **Sheet 数量：** {len(data['sheets'])}
            """)
            
            st.session_state.res_up_res = data['res_up_res']
            st.session_state.res_down_res = data['res_down_res']
            
        except Exception as e:
            st.error(f"文件解析失败: {e}")
            st.session_state.res_input_data = None
    else:
        st.info("请上传 输入.xlsx 文件")
        
        # 提供示例文件下载
        sample_xlsx = Path(__file__).resolve().parent / "data" / "sample" / "输入.xlsx"
    ...
    st.caption("Hydro Toolkit")

# ============================================================
# 主界面
# ============================================================
if st.session_state.res_input_data is None:
    # 未上传文件时显示说明
    st.markdown("""
    ### 👋 欢迎使用水库群调度工具
    ...
    **输入文件格式要求：**
    ...
    with tab2:
        ...
        with col1:
            st.markdown(f"**{data['res_up_res']} 来水**")
            sheet_name = '上游_来水系列'
            if sheet_name in data['sheets']:
        ...
        with col2:
            st.markdown(f"**{data['res_down_res']} 来水**")
            sheet_name = '下游_来水系列'
            if sheet_name in data['sheets']:
                df = data['sheets'][sheet_name]
                st.caption(f"数据量: {len(df)} 条")
        ...
        with col1:
            sheet_name = '上游_调度线'
            if sheet_name in data['sheets']:
                st.markdown(f"**{data['res_up_res']}**")
                df = data['sheets'][sheet_name]
                
                # 绘制图表
                fig = plot_dispatch_line(df, data['res_up_res'])
                if fig:
                    st.plotly_chart(fig, use_container_width=True)
        ...
        with col2:
            sheet_name = '下游_调度线'
            if sheet_name in data['sheets']:
                st.markdown(f"**{data['res_down_res']}**")
                df = data['sheets'][sheet_name]
                
                # 绘制图表
                fig = plot_dispatch_line(df, data['res_down_res'])
                if fig:
                    st.plotly_chart(fig, use_container_width=True)
                ...
                if st.button("🚀 开始计算", type="primary", use_container_width=True):
                    with st.spinner("正在计算，请稍候..."):
                        ...
                        try:
                            ...
                            st.session_state.res_calculated = True
                            st.session_state.res_result_xlsx = result_bytes
                            st.session_state.res_up_res = up_res
                            st.session_state.res_down_res = down_res
    ...
    with tab4:
        ...
        if not st.session_state.res_calculated or st.session_state.res_result_xlsx is None:
            ...
            st.markdown(f"""
            **计算信息：**
            - 上游水库：{st.session_state.res_up_res}
            - 下游水库：{st.session_state.res_down_res}
            - 计算尺度：{calc_step}
            """)
            ...
            with col2:
                st.download_button(
                    label="📥 下载 计算结果.xlsx",
                    data=st.session_state.res_result_xlsx,
            ...
            # 读取结果 xlsx
            result_xlsx = pd.ExcelFile(io.BytesIO(st.session_state.res_result_xlsx))
            
            result_tabs = st.tabs([f"🏔️ {st.session_state.res_up_res}", f"🌊 {st.session_state.res_down_res}"])
...
# 页脚
# ============================================================
footer("水库群调度", repo_url="https://github.com/zengtianli/hydro-reservoir")
